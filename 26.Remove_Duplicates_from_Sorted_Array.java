...
class Solution {
    public int removeDuplicates(int[] nums) {
        ...
        for(int x:nums){
    //Now first check if it is the first element of the 
    //array. If true the just initialize it to the starting position.
            if(pos == 0){
                nums[pos] = x;
                pos++;
            }
            else{
    //If it is not the first element of the array then check if the 
    //current element is greater then the previous element.
                if( x > nums[pos-1]){
                    nums[pos] = x;
                    pos++;
                }
